...
// ============================================================================
// Module: H_Arbiter_Direct
// Description: Arbiter module modified such that data coming from memory goes
//              DIRECTLY into the Datapath without passing through Arbiter registers.
//              Arbiter handles core address decoding, command decoding, and read routing.
// ============================================================================
module H_Arbiter_Direct #(
    ...
    assign wr_a_o = w_addr_valid_i && (w_addr_i == A_BASE_ADDR);
    assign wr_b_o = w_addr_valid_i && (w_addr_i == B_BASE_ADDR);
    assign wr_c_o = w_addr_valid_i && (w_addr_i == C_BASE_ADDR);
    assign wr_d_o = w_addr_valid_i && (w_addr_i == D_BASE_ADDR);
    assign wr_e_o = w_addr_valid_i && (w_addr_i == E_BASE_ADDR);
    assign wr_f_o = w_addr_valid_i && (w_addr_i == F_BASE_ADDR);
    ...
    // Datapath to Output memory connection
    assign output_mem_h_o = datapath_h_i;

    // Read request decode
    assign output_mem_read_enable_o = r_addr_valid_i && (r_addr_i == H_BASE_ADDR);
